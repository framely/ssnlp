...
public interface MatchCostFunction<K,V> {
  ...
  double multiMatchDeltaCost(List<K> k, V v, List<Match<K, V>> prevMultiMatch, List<Match<K, V>> curMultiMatch);

  // pairwise cost of replacing k1 with k2,k3 at position n
  //public double cost(K k1, K k2, K k3, int n);

  abstract class AbstractMatchCostFunction<K,V> implements MatchCostFunction<K,V> {
    // pairwise cost of replacing k1 with k2,k3 at position n
